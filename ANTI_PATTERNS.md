...
| Slides with no identity in the markup | Every slide occupies the same 1920×1080 box, so a review comment, diff or DOM read cannot say which slide it means. | `data-screen-label="NN Label"` on every slide, authored not inherited (`AGENTS.md §14`). |
| A corner mark that vanishes on its own surface | `storyline-navy-white.svg` sounds like the dark variant; all its paths are navy. On navy it is invisible, silently. | Use `storyline-mark.svg` and tint it via CSS `color`. Check `assets/asset-manifest.json` → `surfaceToVariant`. |
| A hand-drawn mark the same colour as the word it marks | Emphasis is coloured and marks default to `currentColor` — both land on lime and the mark disappears into the word. | Word takes the accent; mark takes the surface's other contrast colour. |
| Promising motion with an emoji that doesn't move | `assets/emoji/animated/` is a folder name, not a guarantee; a still ships with no error. | Check `animated-manifest.json` first. If the right emoji doesn't animate, change the emoji. |
| A tap affordance that shouts | A labelled pill on all 20 cards, a pulsing dot, or a coloured offset block — noise, and it breaks the no-coloured-shadow and no-loop rules. | Quiet glyph at rest, motion on hover, one mark per card (`AGENTS.md §15`). |
