...
| Dumping a paragraph or six-line bullet stack | A wall of text can't be read from the room and flattens the slide. | Chunk into 2–4 short labelled beats — stat row, captioned cards, step strip (`Section 5`). |
...
| Slides with no identity in the markup | Every slide occupies the same 1920×1080 box, so a review comment, diff or DOM read cannot say which slide it means. | `data-screen-label="NN Label"` on every slide, authored not inherited (`AGENTS.md Section 14`). |
