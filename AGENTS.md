...
- **Microsoft animates 746 emoji. This repo caches 24 of them.** `microsoft/fluentui-emoji-animated` (MIT) is the library, and it is large. If the emoji you want is not in `assets/emoji/animated/`, that is a gap in *our local cache* — not evidence it doesn't move. Fetch it from upstream. What **`assets/emoji/animated-manifest.json`** tells you is which **local** files genuinely animate: 43 of the original 67 were silently stills and were removed. An APNG carries an `acTL` chunk: `head -c 200 file.png | grep -a acTL` (the `-a` is load-bearing — without it grep treats the binary as non-matching and reports everything as a still).
