...
- **Clearing WCAG is the floor, not the goal — check that a pairing also *reads* at display size.** `preview/brand-color-pairings.html` marks a pair valid on contrast ratio alone, and a ratio says nothing about how a colour behaves at 120px across a room. Teal `#145F7B` on aqua `#86FFF1` clears AA and still read flat and weak projected; purple `#6103B9` on the same aqua was dramatically stronger. Two mid-luminance brand colours together will usually pass and usually underperform — when a pair is the *point* of a slide (a display headline, a statement word, a hero stat), pick the one with the widest luminance gap you can get, not merely the first that passes.
...
- **Only 24 of the 67 curated emoji actually animate — check before you promise motion.** `assets/emoji/animated/` is a folder name, not a guarantee. 43 files that were stills have been removed; **`assets/emoji/animated-manifest.json` is the machine-readable list** of what genuinely moves. An APNG carries an `acTL` chunk: `head -c 200 file.png | grep -a acTL` (the `-a` is load-bearing — without it grep treats the binary as non-matching and reports everything as a still).
- **When the peak's obvious emoji doesn't animate, change the emoji — not the doctrine.** `light-bulb`, `sparkles`, `direct-hit`, `trophy`, `warning`, `memo`, `package` and `seedling` do **not** move. Pick a neighbour that carries the same beat and does: idea → `high-voltage` / `glowing-star` / `brain`; target hit → `hundred-points`; launch or delivery → `rocket`. If nothing animated fits the meaning, ship the 3D still and spend the motion budget on another slide. A wrong-meaning emoji that moves is worse than a right-meaning one that doesn't.
- **Icons are a different job from emoji — and they have their own set.** For *functional* symbols (wayfinding, list markers, labels, dense layouts) use **`assets/icons/`**: 1,595 localised Fluent **Flat** icons, the system's symbol vocabulary. Emoji are for the *emotional* beat; icons are for the functional one. An icon is something you look up because you need the one that means "database"; an emoji is something you choose because a slide needs to feel like something. Never press an emoji into icon duty just because it was to hand, and never recolour a Flat icon — its colours are the artwork.
...
- **Prefer `transform` and `opacity`.** They composite; `width`, `height`, `padding` and `margin` trigger layout. **The documented exception:** a size transition *inside a fixed-size container* reflows nothing outside itself, so an expander whose parent height is pinned may animate `max-height` — the card's own footprint never changes and no sibling moves. Outside a fixed container, animate a transform instead.
