...
- **Style priority — treat Fluent emoji as brand assets, not decoration:** for *visual storytelling* (hero moments, key beats, stat/feature markers) use **3D first** (`variant="3d"`) — the default; **mix in animated 3D** (`variant="animated"`) where it adds life, reserving it for the *one* emotional peak — the cover's idea, the hero stat, the closing line (one animated emoji per few slides, never a wall of motion). **2D Color** (`variant="color"`) is the fallback when 3D doesn't suit the surface. For **icon/utility duty** — wayfinding, dense layouts, monochrome contexts — use **Flat** (`variant="flat"`): a comprehensive, clean icon set that serves as the full symbol vocabulary when a flat look is wanted.
- **Sourcing SOP:** **local/offline deliverables** (native PowerPoint/Keynote, embedded PDFs, anything that must render without a network) → **always download the asset from `github.com/microsoft/fluentui-emoji` and embed it as a file** — PNG for static 3D/Flat, GIF/APNG for animated. Never hotlink. **Web deliverables** → pulling from a CDN (jsDelivr) is fine; the injector already resolves local-first → CDN. **Never use raw platform Unicode emoji as icons** — they render differently per OS; always the actual Fluent asset.
