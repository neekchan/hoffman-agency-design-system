...
- **The storyline line on navy is a bookend, not a surface.** Reserve the full-frame lime line for the **cover and closing** (≤2 slides per deck); never use it as a content-slide background (statements, agendas, columns, personas, data, tables, quotes). A navy content slide is a plain navy field — no line. Repeating it across slides makes every slide read like a cover and flattens the deck. (The boxed corner monogram on light/non-navy surfaces is a separate device — one per slide.)
