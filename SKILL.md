...
- **Type**: **Poppins** (sans — workhorse for everything: headlines, body, UI). **Libre Baskerville — ITALIC ONLY** for emphasis (the key word or phrase inside a Poppins headline, pull-quotes). Never use Baskerville upright. **M PLUS 2** for Japanese, **Noto Sans** family for CJK / other languages. JetBrains Mono for captions/stats. Huge display sizes. (Whitespace: *generous* on web; *fill the frame* on slides — see the medium split below.)
...
- **⚠ MEDIUM SPLIT — web vs. slides**: This system serves two media with different layout physics. **Web / social** (webpages, UI kit, tiles) follows `DESIGN.md` + `colors_and_type.css` — web type scale (16px body), generous whitespace, sparse imagery. **Slides / office docs** (1920×1080 decks) follow `AGENTS.md` + `LAYOUTS.md` and **OVERRIDE the web rules**: a dedicated slide type scale as FLOORS, biased high (20–24px labels · 30–36px body / default 32 · 40–52px subhead · 64–80px title · 120–132px statement/divider · 176px+ cover · ~240px closing word — never the old 28px "safe" body), "fill the frame" (no dead whitespace), and **imagery by default on every slide**. Color, fonts, the italic-emphasis move, the Storyline motif, annotations, and voice are brand-wide and apply to both. When building a deck, never inherit web body sizes, web max-widths, or "sparse imagery."
- **🗒 NATIVE POWERPOINT — when the output is a `.pptx` file**: read `POWERPOINT.md` first. Prefer exporting the HTML deck (`templates/deck/Deck.dc.html`) to editable PPTX; hand-build (python-pptx etc.) only when a tool can't, using `assets/asset-manifest.json` (logo geometry + protected zones + surface→variant) and `templates/powerpoint/layout-manifest.json` (the 48 layout contracts). Confirm Poppins + Libre Baskerville Italic are available to the tool and set the PowerPoint **theme** fonts to Poppins (files present ≠ used); place logos by the manifest ratio (lime → navy logo); layout codes are contracts; every arrow connects two named elements; validate the finished file (`POWERPOINT.md §9`).
- **⚡ DECK MODE — Presenter XOR Document**: every deck is exactly one mode. **Presenter** (live) = ≤1 idea, ≤15 words/slide, image-led, detail in **speaker notes**; **Document** (leave-behind, stands alone) = denser, hierarchical, short bullets allowed. Never mix the two. Infer the mode from context, state the assumption, ask only if genuinely ambiguous. Both modes keep the full Hoffman visual system (fill-the-frame, slide scale, whole palette) — mode changes *depth*, not appearance. The `templates/deck/` library carries the **Mode** tweak; the full worked demo of the system is `slides/Hoffman Brand Guidelines.html`. See `AGENTS.md §12`.
